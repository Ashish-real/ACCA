/* ==========================================================================
   ...
   Zero text-overlapping with dynamic line-splitting and precise bounding boxes.
   ========================================================================== */
...
const PdfCheatSheetEngine = {
  ...
  renderCard(doc, x, y, colW, title, bullets, tag = "") {
    ...
    // Split every bullet into its wrapped visual lines
    const lineList = [];
    bullets.forEach(b => {
      const splitLines = doc.splitTextToSize(b, innerW);
      splitLines.forEach(sl => lineList.push(sl));
    });

    const cardHeight = 8 + (lineList.length * lineHeight) + 2.5;
  ...
  generateFMPDF() {
    ...
    y2 += this.renderCard(doc, col2X, y2, colW, "Foreign Exchange Hedging (Forex)", [
      ...
      "  - Receipt (Future Foreign Inflow): Borrow Foreign → Spot → Deposit Home.",
      "  - Payment (Future Foreign Outflow): Borrow Home → Spot → Deposit Foreign.",
